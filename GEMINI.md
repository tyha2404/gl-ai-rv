...
Automated code reviews for GitLab Merge Requests powered by AI (9Router AI Gateway / OpenAI-compatible proxy).
...
- **AI Integration:** `openai` SDK (connecting to 9Router AI Gateway / OpenAI-compatible endpoint)
...
- `src/ai.ts`: `AIClient` class for interacting with the AI model via 9Router gateway. It prepares prompts, sends diffs, and parses the JSON response.
...
- 9Router instance (or OpenAI-compatible API Gateway)
